...
namespace RobotAPI
{
	public static partial class Robot
	{
		...
		public static decimal LOW   { get; private set; }
		...
		public static decimal PN(int v) =>                      // Получение сдвига цены на основании количества пунктов
			(decimal)v / format.Exp(INSTRUMENT.Decimals);
		...
		static void CandleGlobalSet()
		{
			...
			UNIX   = CANDLES().Unix;
			PRICE  = (decimal)CANDLES().Close;
			HIGH   = CANDLES().High;
			OPEN   = CANDLES().Open;
			CLOSE  = CANDLES().Close;
			LOW    = (decimal)CANDLES().Low;
